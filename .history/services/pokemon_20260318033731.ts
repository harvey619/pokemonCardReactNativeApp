...
export async function fetchPokemonList(limit = 100): Promise<PokemonCardSummary[]> {
  ...
  const detailedPokemons: PokemonCardSummary[] = [];
  const batchSize = 30;
  ...
  for (let i = 0; i < data.results.length; i += batchSize) {
    ...
    detailedPokemons.push(...batchDetails);
  }

  return detailedPokemons;
}

export async function fetchPokemonPage(limit = 60, offset = 0): Promise<PokemonListPage> {
  const response = await fetch(`${POKE_API_BASE_URL}/pokemon?limit=${limit}&offset=${offset}`);
  if (!response.ok) {
    throw new Error("Unable to load pokemon page");
  }
...
export async function fetchAllPokemonList(): Promise<PokemonCardSummary[]> {
  const countResponse = await fetch(`${POKE_API_BASE_URL}/pokemon?limit=1`);
  if (!countResponse.ok) {
    throw new Error("Unable to load pokemon count");
  }

  const countData = (await countResponse.json()) as PokemonListResponse;
  ...
  return fetchPokemonList(totalCount);
}
